...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestCDCLagMetric)) {
  ...
  // Call get changes.
  GetChangesResponsePB change_resp;
  ASSERT_OK(WaitForGetChangesToFetchRecords(&change_resp, stream_id[0], tablets, 2));
...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestAddTableAfterDropTableAfterUpdateStreamState)) {
  ...
  // create a new table and verify that it never gets added to stream metadata even after state
  // change.
  table[idx] = ASSERT_RESULT(CreateTable(
  ...
  // Stream metadata should not contain the newly created table since it would be removed from
  // unprocessed map.
...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestLogGCForNewTablesAddedAfterCreateStream)) {
  ...
  // Restart of the tsever will clear WAL cache
  test_cluster()->mini_tablet_server(0)->Shutdown();
  ASSERT_OK(test_cluster()->mini_tablet_server(0)->Start());
...
TEST_F(
  ...
  for (uint32_t idx = 0; idx < num_tables; idx++) {
    ...
    ASSERT_OK(test_client()->FlushTables(
        ...
        /* is_compaction = */ false));
    GetChangesResponsePB change_resp;
    ASSERT_OK(WaitForGetChangesToFetchRecords(&change_resp, stream_id[idx], tablets[idx], 99));
...
TEST_F(CDCSDKYsqlTest, YB_DISABLE_TEST_IN_TSAN(TestCDCSDKMetricsTwoTablesTwoStreamsOnBothTables)) {
  ...
  for (uint32_t idx = 0; idx < num_tables; idx++) {
    ...
    for (uint32_t stream_idx = 0; stream_idx < num_streams; stream_idx++) {
      GetChangesResponsePB change_resp;
      ASSERT_OK(
          WaitForGetChangesToFetchRecords(&change_resp, stream_id[stream_idx], tablets[idx], 99));
