...
Status CatalogManager::CreateCDCStream(
  ...
  // CDCSDK only.
    ...
    RETURN_NOT_OK(ValidateCDCSDKRequestProperties(
        *req, source_type_option_value, id_type_option_value));
...
Status CatalogManager::RollbackFailedCreateCDCSDKStream(
  ...
  switch (cdcsdk_stream_creation_state) {
    case CDCSDKStreamCreationState::kAddedToMaps: {
      LockGuard lock(mutex_);
      RETURN_NOT_OK(CleanUpCDCSDKStreamFromMaps(stream));
      break;
    }
    ...
    for (auto& [delete_stream_id, drop_table_list] : drop_stream_tablelist) {
      if (cdc_stream_map_.contains(delete_stream_id)) {
        ...
        if (ltm->table_id().size() == static_cast<int>(drop_table_list.size())) {
          RETURN_NOT_OK(CleanUpCDCSDKStreamFromMaps(cdc_stream_info));
...
void CatalogManager::AddCDCStreamToUniverseAndInitConsumer(
  ...
  std::vector<XClusterConsumerStreamInfo> consumer_info;
  {
    ...
    auto status = sys_catalog_->Upsert(leader_ready_term(), universe);
    if (!status.ok()) {
      LOG(WARNING) << "An error occurred while updating universe replication info in sys-catalog: "
                   << status;
      return;
    }
  ...
  if (merge_alter) {
    auto status = MergeUniverseReplication(universe, final_id, std::move(on_success_cb));
    if (!status.ok()) {
      MarkUniverseReplicationFailed(universe, status);
      return;
    }
...
Status CatalogManager::RemoveTablesFromReplication(
...

  {
    ...
    SyncXClusterConsumerReplicationStatusMap(
        cdc::ReplicationGroupId(req->producer_id()), *pm);
...
Status CatalogManager::UpdateConsumerOnProducerSplit(
  ...
  SyncXClusterConsumerReplicationStatusMap(
      cdc::ReplicationGroupId(req->producer_id()), *replication_group_map);
  l.Commit();
...
Status CatalogManager::GetReplicationStatus(
  ...
  if (!req->universe_id().empty()) {
    return PopulateReplicationGroupErrors(
        cdc::ReplicationGroupId(req->universe_id()), resp);
  }
...
Status CatalogManager::DisableDynamicTableAdditionOnCDCSDKStream(
  ...
  if (!req->has_stream_id()) {
    RETURN_INVALID_REQUEST_STATUS("CDC Stream ID must be provided", (*req));
  }

  if (!FLAGS_cdcsdk_enable_dynamic_tables_disable_option) {
    RETURN_INVALID_REQUEST_STATUS(
        "Disabling addition of dynamic tables to CDC stream is disallowed in the middle of an "
        "upgrade. Finalize the upgrade and try again", (*req));
  }
  ...
  if (!stream->IsCDCSDKStream()) {
    RETURN_INVALID_REQUEST_STATUS("Not a CDC stream", (*req));
  }
...
Status CatalogManager::RemoveUserTableFromCDCSDKStream(
  ...
  if (!req->has_stream_id() || !req->has_table_id()) {
    RETURN_INVALID_REQUEST_STATUS("Both CDC Stream ID and table ID must be provided", (*req));
  }
  ...
  if (!stream->GetCdcsdkYsqlReplicationSlotName().empty()) {
    RETURN_INVALID_REQUEST_STATUS(
        "Cannot remove table from CDC streams that are associated with a replication slot", (*req));
  }
  ...
  if (!stream->IsDynamicTableAdditionDisabled()) {
    RETURN_INVALID_REQUEST_STATUS(
        ...
        "dynamic table addition on the stream.", (*req));
  }
...

  {
    ...
    if (!IsTableEligibleForCDCSDKStream(table, schema)) {
      RETURN_INVALID_REQUEST_STATUS(
          "Only allowed to remove user tables from CDC streams via this command.", (*req));
    }
  }

  auto table_ns_id = table->LockForRead()->namespace_id();
  if (table_ns_id != stream_ns_id) {
    RETURN_INVALID_REQUEST_STATUS("Stream and Table are not under the same namespace", (*req));
  }
