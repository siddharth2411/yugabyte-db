...
Status PgApiImpl::CDCGetChanges(YBCCDCSDKCheckpoint* cdcsdk_checkpoint, YBCGetChangesResponse* response) {
  ...
  if (change_resp.has_error()) {
    LOG(INFO) << "Sid: Response error: " << change_resp.error().status().errors();
  }
