...
Status GetUniverseConfig(ClusterAdminClient* client,
...
Status ClusterAdminCli::Run(int argc, char** argv) {
  ...
  std::unique_ptr<ClusterAdminClient> client;
  ...
  if (!FLAGS_init_master_addrs.empty()) {
    ...
    RETURN_NOT_OK(HostPort::ParseStrings(
        ...
        &init_master_addrs));
    client.reset(new ClusterAdminClient(
        init_master_addrs[0],
...
Result<rapidjson::Document> DdlLog(
    ClusterAdminClient* client, const ClusterAdminCli::CLIArguments& args) {
  RETURN_NOT_OK(CheckArgumentsCount(args.size(), 0, 0));
  return client->DdlLog();
}

void ClusterAdminCli::RegisterCommandHandlers(ClusterAdminClient* client) {
  ...
  Register("list_snapshots", std::move(options), [client](const CLIArguments& args) -> Status {
  ...
  Register(
      ...
      [client](const CLIArguments& args) -> Status {
        ...
        RETURN_NOT_OK_PREPEND(
            ...
            Substitute("Unable to create snapshot of tables: $0", yb::ToString(tables)));
        return Status::OK();
      });
  ...
  RegisterJson(
      ...
      [client](const CLIArguments& args) -> Result<rapidjson::Document> {
        if (args.size() != 3 && args.size() != 5) {
          return STATUS(
              InvalidArgument, Format("Expected 3 or 5 arguments, received $0", args.size()));
        }
        auto schedule_id = VERIFY_RESULT(SnapshotScheduleId::FromString(args[0]));
  ...
  Register(
      ...
      [client](const CLIArguments& args) -> Status {
        ...
        RETURN_NOT_OK_PREPEND(
            ...
            Substitute("Unable to create snapshot of keyspace: $0", keyspace.name));
        return Status::OK();
      });
  ...
  Register(
      ...
      [client](const CLIArguments& args) -> Status {
        ...
        RETURN_NOT_OK_PREPEND(
            ...
            Substitute("Unable to create snapshot of database: $0", database.name));
        return Status::OK();
      });
  ...
  Register(
      ...
      [client](const CLIArguments& args) -> Status {
        ...
        RETURN_NOT_OK_PREPEND(
            ...
            Substitute("Unable to export snapshot $0 to file $1", snapshot_id, file_name));
        return Status::OK();
      });
  ...
  Register("delete_snapshot", " <snapshot_id>", [client](const CLIArguments& args) -> Status {
    ...
    const string snapshot_id = args[0];
    RETURN_NOT_OK_PREPEND(
        client->DeleteSnapshot(snapshot_id),
        Substitute("Unable to delete snapshot $0", snapshot_id));
    return Status::OK();
  });

  Register("list_replica_type_counts", " <table>", [client](const CLIArguments& args) -> Status {
    const auto table_name = VERIFY_RESULT(ResolveSingleTableName(client, args.begin(), args.end()));
    RETURN_NOT_OK_PREPEND(
  ...
  Register("rotate_universe_key", " <key_path>", [client](const CLIArguments& args) -> Status {
    if (args.size() < 1) {
      return ClusterAdminCli::kInvalidArguments;
    }
  ...
  Register("disable_encryption", "", [client](const CLIArguments& args) -> Status {
    RETURN_NOT_OK_PREPEND(client->DisableEncryption(), "Unable to disable encryption.");
    return Status::OK();
  });

  Register("is_encryption_enabled", "", [client](const CLIArguments& args) -> Status {
  ...
  Register(
      "rotate_universe_key_in_memory", " <key_id>", [client](const CLIArguments& args) -> Status {
        if (args.size() != 1) {
          return ClusterAdminCli::kInvalidArguments;
        }
        string key_id = args[0];
  ...
  Register("disable_encryption_in_memory", "", [client](const CLIArguments& args) -> Status {
    if (args.size() != 0) {
      return ClusterAdminCli::kInvalidArguments;
    }
  ...
  Register("create_cdc_stream", " <table_id>", [client](const CLIArguments& args) -> Status {
    if (args.size() < 1) {
      return ClusterAdminCli::kInvalidArguments;
    }
    const string table_id = args[0];
  ...
  Register(
      "delete_change_data_stream", " <db_stream_id>", [client](const CLIArguments& args) -> Status {
        if (args.size() < 1) {
          return ClusterAdminCli::kInvalidArguments;
        }
        ...
        RETURN_NOT_OK_PREPEND(
            ...
            Substitute("Unable to delete CDC database stream id $0", db_stream_id));
        return Status::OK();
      });

  Register("list_cdc_streams", " [<table_id>]", [client](const CLIArguments& args) -> Status {
  ...
  Register(
      "list_change_data_streams", " [<namespace>]", [client](const CLIArguments& args) -> Status {
        if (args.size() != 0 && args.size() != 1) {
          return ClusterAdminCli::kInvalidArguments;
        }
  ...
  Register(
      ...
      [client](const CLIArguments& args) -> Status {
        ...
        RETURN_NOT_OK_PREPEND(
            ...
            Substitute("Unable to list CDC stream info for database stream $0", db_stream_id));
        return Status::OK();
      });
  ...
  Register(
      ...
      [client](const CLIArguments& args) -> Status {
        ...
        RETURN_NOT_OK_PREPEND(
            ...
            Substitute("Unable to delete replication for universe $0", producer_id));
        return Status::OK();
      });
  ...
  Register(
      "change_xcluster_role", " <STANDBY|ACTIVE>", [client](const CLIArguments& args) -> Status {
        if (args.size() != 1) {
          return ClusterAdminCli::kInvalidArguments;
        }
        ...
        return STATUS(
            InvalidArgument, Format("Expected one of STANDBY OR ACTIVE, found $0", args[0]));
      });
  ...
  Register(
      ...
      [client](const CLIArguments& args) -> Status {
        ...
        RETURN_NOT_OK_PREPEND(
            client->BootstrapProducer(table_ids), "Unable to bootstrap CDC producer");
        return Status::OK();
      });
  ...
  Register(
      ...
      Format(
          " <comma_separated_list_of_stream_ids>"
...
Result<client::YBTableName> ResolveSingleTableName(ClusterAdminClient* client,
                                                   CLIArgumentsIterator i,
                                                   const CLIArgumentsIterator& end,
